...
void MpvWidget::seek(double seconds) {
    ...
    // Convert the double to a string for MPV's command interface.
    // std::to_string() is a C++ standard library function.
    std::string timeStr = std::to_string(seconds);

    // The "seek" command with "relative" mode seeks relative to current position.
    // (Other modes: "absolute", "absolute-percent", etc.)
